...
static void beep(uint16_t freq, uint16_t dur) {
  if (settings().sound) compat::beepTone(freq, dur);
}
...
static void clockRefreshRtc() {
  ...
  _clkLastRead = millis();
  _onUsb = compat::getVBusVoltageV() > 4.0f;
...
void drawInfo() {
  ...
  if (infoPage == 0) {
    ...
    int vBat_mV = (int)(compat::getBatteryVoltageV() * 1000);
    int iBat_mA = (int)compat::getBatteryCurrentMa();
    int vBus_mV = (int)(compat::getVBusVoltageV() * 1000);
    ...
    ln("  temp     %dC", (int)compat::getInternalTempC());
...
void loop() {
  ...
  // M5StickS3 has no user-controllable LED, so chirp on the transition
  // into attention as an audible substitute. Gated on settings().led so
  // the same toggle silences both the (future) LED pulse and this beep.
  static PersonaState lastActiveState = P_SLEEP;
  if (activeState == P_ATTENTION && lastActiveState != P_ATTENTION && settings().led) {
    beep(1800, 120);
  }
  ...
  if (compat::getPowerBtnPress() == 0x02) {
    if (screenOff) {
      wake();
    } else {
      compat::setAuxiliaryRail(false);
      screenOff = true;
    }
  }
